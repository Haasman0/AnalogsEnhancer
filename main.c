...
static uint32_t innerDZLA, outerDZLA, innerDZRA, outerDZRA;
static char buffer[32];
static char widePatch;
static uint8_t apply_wide_patch = 0;
 
static void (*patchFuncLeft)(uint8_t *x, uint8_t *y, int inner, int outer);
static void (*patchFuncRight)(uint8_t *x, uint8_t *y, int inner, int outer);
 
// Courtesy of rsn8887
void rescaleAnalogs(uint8_t *x, uint8_t *y, int inner, int outer) {
  ...
  // should probably add protections for outer deadzone being too high but
  // ....eh....
 
  float analogX = (float) *x - 127.0f;
  float analogY = (float) *y - 127.0f;
  float innerDZ = (float) inner;
  float outerDZ = (float) outer;
  float magnitude = sqrt(analogX * analogX + analogY * analogY);
  
  if (magnitude >= innerDZ){
    ...
    float scalingFactor = maximum / magnitude * (magnitude - innerDZ) / ((maximum-outerDZ) - innerDZ);
    ...
    float rx = ((clampingFactor * analogX) + 127.0f);
    float ry = ((clampingFactor * analogY) + 127.0f);
    
    if (rx >= 254.0) rx = 255.0;
    if (ry >= 254.0) ry = 255.0;
...
void patchData(uint8_t *data) {
    patchFuncLeft(&data[12], &data[13], innerDZLA, outerDZLA);
    patchFuncRight(&data[14], &data[15], innerDZRA, outerDZRA);
}
...
void loadConfig(void) {
    ...
    if (fd >= 0){
        ...
        ksceIoClose(fd);
    }else sprintf(buffer, "left=30,15;right=30,15;y");
    sscanf(buffer, "left=%i,%i;right=%i,%i;%c", &innerDZLA, &outerDZLA, &innerDZRA, &outerDZRA, &widePatch);
    
    patchFuncLeft = rescaleAnalogs;
    patchFuncRight = rescaleAnalogs;
